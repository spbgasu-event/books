...
function openBook(id){
  ...
  requestAnimationFrame(() => {
    ...
    R.pageIdx = paraIdxToPageIndex(prog.paraIdx || 0);
    renderCurrentPage();
  });
}
 ...
 * Если paraIdx больше последнего абзаца — возвращает индекс ПОСЛЕДНЕЙ страницы,
 * НО зажимаем по длине главы — это критично для переключения языков.
 */
function paraIdxToPageIndex(paraIdx){
  if (!R.pages.length) return 0;
  // Зажимаем в пределах главы — не даём «утечь» за последний абзац
  const maxPara = R.totalParas - 1;
  const safeParaIdx = Math.max(0, Math.min(maxPara, paraIdx));

  let bestIdx = 0;
  for (let i = 0; i < R.pages.length; i++){
    if (R.pages[i].paraFrom <= safeParaIdx) bestIdx = i;
    else break;
  }
  return bestIdx;
}
