...
function renderLibrary(){
  ...
  g.innerHTML = S.books.map(b => {
    ...
    const total = (b.enChaps?.length || b.ruChaps?.length || 1);
    const overallPct = Math.round(((prog.chapIdx + (prog.anchorRatio||0)) / total) * 100);
...
async function addBook(){
  ...
  try {
    ...
    const book = {
      ...
      progress: {chapIdx:0, lang:'en', anchorRatio:0},
      addedAt: Date.now()
    };
    S.books.push(book);
    await saveBook(book);
